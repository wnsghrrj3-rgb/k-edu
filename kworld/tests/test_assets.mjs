...
for (const [rel, a] of Object.entries(man.assets)) {
  ...
  if (a.kind === 'person' || a.kind === 'animal') { const names = (j.animations || []).map((x) => x.name.toLowerCase()); ok(names.some((n) => n.includes('wait') || n.includes('idle')) && names.some((n) => n.includes('walk')), `${rel} 대기·걷기 동작`); ok((j.skins || []).length === 1, `${rel} 뼈대 1`); }
