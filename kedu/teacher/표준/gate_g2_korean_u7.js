/* gate_g2_korean_u7.js — L4 게이트 (g2 국어 u7 「마음을 담아서 말해요」 국어 §7 증보 15차시).
   ...
   실행: NODE_PATH=/home/claude/.jsdom/node_modules node gate_g2_korean_u7.js */
'use strict';
const fs = require('fs');
...
const DATA = fs.readFileSync(path.join(TDIR, 'data/g2_korean_u7.js'), 'utf8');
...
const MAP = Array.from({ length: 15 }, (_, i) => ['u7_l' + String(i + 1).padStart(2, '0'), String(i + 1)]);
...
T('부팅 + u7 15차시 로드', () => {
  ...
  ok(keys.length === 15, 'u7 차시 ' + keys.length);
});
...
for (const [key, arg] of MAP) {
  T(key + ' 요소 렌더', () => {
    ...
    ok(/고운 말|마음|발표|자세|대화|들/.test(ALL), '③ 고운 말/마음 서사 흔적 없음');
  });
}
...
T('저작권 가드 — 지도서 제재 미노출', () => {
  ...
  delete require.cache[require.resolve(path.join(TDIR, 'data/g2_korean_u7.js'))];
  require(path.join(TDIR, 'data/g2_korean_u7.js'));
  const L = global.window.LESSONS;
  const BANNED_SRC = ['메기야, 고마워', '지우와 머리핀', '왜 지각했냐면요', '고운 말 동요'];
  const bad = [];
...
T('review 존재(l01 제외 14차시)·exit 전 차시 존재', () => {
  ...
  require(path.join(TDIR, 'data/g2_korean_u7.js'));
  const L = global.window.LESSONS;
  const badRv = Object.keys(L).filter(k => k.startsWith('u7_') && !NO_REVIEW.includes(k)).filter(k => !L[k].slides.some(s => s.block === 'review' && s.data.items));
  const badEx = Object.keys(L).filter(k => k.startsWith('u7_')).filter(k => !L[k].slides.some(s => s.block === 'exit_ticket'));
