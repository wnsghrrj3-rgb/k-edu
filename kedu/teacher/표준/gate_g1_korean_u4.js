/* gate_g1_korean_u4.js — L3 게이트 (g1 국어 u4 「여러 가지 낱말을 익혀요」 국어 §7 증보 14차시).
   ...
   실행: NODE_PATH=/home/claude/.jsdom/node_modules node gate_g1_korean_u4.js */
'use strict';
...
const DATA = fs.readFileSync(path.join(TDIR, 'data/g1_korean_u4.js'), 'utf8');
...
const MAP = Array.from({ length: 14 }, (_, i) => ['u4_l' + String(i + 1).padStart(2, '0'), String(i + 1)]);
...
T('부팅 + u4 14차시 로드', () => {
  ...
  ok(keys.length === 14, 'u4 차시 ' + keys.length);
});
...
for (const [key, arg] of MAP) {
  T(key + ' 요소 렌더', () => {
    ...
    ok(/낱말|이름|말|또박또박|몸|가족/.test(ALL), '③ 단원 서사(낱말·이름) 흔적 없음');
  });
}
...
T('교과서 제재 표지 전 차시 미노출 (저작권 가드 자체검증)', () => {
  const BANNED_SRC = ['설문대', '누리호', '소율이', '요리 일기', '이게 뭐예요', '쓰레기가 모여'];
  global.window = { LESSONS: {} }; global.LESSONS = global.window.LESSONS;
  delete require.cache[require.resolve(path.join(TDIR, 'data/g1_korean_u4.js'))];
  require(path.join(TDIR, 'data/g1_korean_u4.js'));
