/* gate_g2_korean_u3.js — L4 게이트 (g2 국어 u3 「겪은 일을 나타내요」 국어 §7 증보 15차시).
   ...
   실행: NODE_PATH=/home/claude/.jsdom/node_modules node gate_g2_korean_u3.js */
'use strict';
const fs = require('fs');
...
const DATA = fs.readFileSync(path.join(TDIR, 'data/g2_korean_u3.js'), 'utf8');
...
const MAP = Array.from({ length: 15 }, (_, i) => ['u3_l' + String(i + 1).padStart(2, '0'), String(i + 1)]);
const NO_REVIEW = ['u3_l01'];
...
T('부팅 + u3 15차시 로드', () => {
  ...
  ok(keys.length === 15, 'u3 차시 ' + keys.length);
});
...
for (const [key, arg] of MAP) {
  T(key + ' 요소 렌더', () => {
    ...
    ok(/꾸미|겪은|일기|문장|느낌/.test(ALL), '③ 겪은 일 서사 흔적 없음');
  });
}
...
T('저작권 가드 — 지도서 제재 미노출', () => {
  ...
  delete require.cache[require.resolve(path.join(TDIR, 'data/g2_korean_u3.js'))];
  require(path.join(TDIR, 'data/g2_korean_u3.js'));
  const L = global.window.LESSONS;
  const BANNED_SRC = ['식물은 어떻게 자랄까', '누리호', '소율이', '요리 일기', '이게 뭐예요'];
  const bad = [];
...
T('offline 짝 활동 전 차시 존재(국어 필수 1)', () => {
  ...
  require(path.join(TDIR, 'data/g2_korean_u3.js'));
  const L = global.window.LESSONS;
  const bad = Object.keys(L).filter(k => k.startsWith('u3_')).filter(k => !L[k].slides.some(s => s.block === 'offline_activity'));
...
T('review 존재(l01 제외 14차시)·exit 전 차시 존재', () => {
  ...
  require(path.join(TDIR, 'data/g2_korean_u3.js'));
  const L = global.window.LESSONS;
  const badRv = Object.keys(L).filter(k => k.startsWith('u3_') && !NO_REVIEW.includes(k)).filter(k => !L[k].slides.some(s => s.block === 'review' && s.data.items));
  const badEx = Object.keys(L).filter(k => k.startsWith('u3_')).filter(k => !L[k].slides.some(s => s.block === 'exit_ticket'));
