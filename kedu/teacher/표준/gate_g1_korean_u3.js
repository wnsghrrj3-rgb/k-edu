/* gate_g1_korean_u3.js — L3 게이트 (g1 국어 u3 「낱말과 친해져요」 국어 §7 증보 13차시).
   ...
   실행: NODE_PATH=/home/claude/.jsdom/node_modules node gate_g1_korean_u3.js */
'use strict';
...
const DATA = fs.readFileSync(path.join(TDIR, 'data/g1_korean_u3.js'), 'utf8');
...
const MAP = Array.from({ length: 13 }, (_, i) => ['u3_l' + String(i + 1).padStart(2, '0'), String(i + 1)]);
...
T('부팅 + u2 13차시 로드', () => {
  ...
  ok(keys.length === 13, 'u3 차시 ' + keys.length);
});
...
for (const [key, arg] of MAP) {
  T(key + ' 요소 렌더', () => {
    ...
    ok(/받침|글자|낱말|자음자|또박또박/.test(ALL), '③ 단원 서사(받침 완성·자음자·낱말) 흔적 없음');
  });
}
...
T('결합 사실 테이블([글자]＋[받침]＝[받침 글자]) 일치', () => {
  const FACTS = [['다', 'ㄹ', '달'], ['무', 'ㄹ', '물'], ['채', 'ㄱ', '책'], ['바', 'ㄱ', '박'], ['차', 'ㅇ', '창'], ['느', 'ㄹ', '늘'], ['고', 'ㅁ', '곰'], ['가', 'ㅇ', '강'], ['고', 'ㅇ', '공'], ['구', 'ㄹ', '굴']];
...
T('증보 leveled·exit·review 내부 결합 무모순 (전수 검산)', () => {
  ...
  delete require.cache[require.resolve(path.join(TDIR, 'data/g1_korean_u3.js'))];
  require(path.join(TDIR, 'data/g1_korean_u3.js'));
