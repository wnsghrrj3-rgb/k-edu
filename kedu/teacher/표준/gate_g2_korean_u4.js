/* gate_g2_korean_u4.js — L4 게이트 (g2 국어 u4 「분위기를 살려 읽어요」 국어 §7 증보 15차시).
   ...
   실행: NODE_PATH=/home/claude/.jsdom/node_modules node gate_g2_korean_u4.js */
'use strict';
const fs = require('fs');
...
const DATA = fs.readFileSync(path.join(TDIR, 'data/g2_korean_u4.js'), 'utf8');
...
const MAP = Array.from({ length: 15 }, (_, i) => ['u4_l' + String(i + 1).padStart(2, '0'), String(i + 1)]);
const NO_REVIEW = ['u4_l01'];

function renderAll(w, lessonArg) {
  w.Teacher.openShow('4', String(lessonArg));
...
T('부팅 + u4 15차시 로드', () => {
  ...
  ok(keys.length === 15, 'u4 차시 ' + keys.length);
});
...
for (const [key, arg] of MAP) {
  T('회귀 ' + key, () => {
    const W2 = boot();
    W2.Teacher.openShow('4', String(arg));
...
T('저작권 가드 — 지도서 제재 미노출', () => {
  ...
  delete require.cache[require.resolve(path.join(TDIR, 'data/g2_korean_u4.js'))];
  require(path.join(TDIR, 'data/g2_korean_u4.js'));
  const L = global.window.LESSONS;
  const BANNED_SRC = ['설문대', '쓰레기가 모여'];
  const bad = [];
  Object.keys(L).filter(k => k.startsWith('u4_')).forEach(k => {
...
T('offline 짝 활동 전 차시 존재(국어 필수 1)', () => {
  ...
  require(path.join(TDIR, 'data/g2_korean_u4.js'));
  const L = global.window.LESSONS;
  const bad = Object.keys(L).filter(k => k.startsWith('u4_')).filter(k => !L[k].slides.some(s => s.block === 'offline_activity'));
...
T('review 존재(l01 제외 14차시)·exit 전 차시 존재', () => {
  ...
  require(path.join(TDIR, 'data/g2_korean_u4.js'));
  const L = global.window.LESSONS;
  const badRv = Object.keys(L).filter(k => k.startsWith('u4_') && !NO_REVIEW.includes(k)).filter(k => !L[k].slides.some(s => s.block === 'review' && s.data.items));
  const badEx = Object.keys(L).filter(k => k.startsWith('u4_')).filter(k => !L[k].slides.some(s => s.block === 'exit_ticket'));
