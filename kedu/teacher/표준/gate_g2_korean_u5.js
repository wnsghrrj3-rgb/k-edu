/* gate_g2_korean_u5.js — L4 게이트 (g2 국어 u5 「마음을 짐작해요」 국어 §7 증보 14차시).
   ...
   실행: NODE_PATH=/home/claude/.jsdom/node_modules node gate_g2_korean_u5.js */
'use strict';
const fs = require('fs');
...
const DATA = fs.readFileSync(path.join(TDIR, 'data/g2_korean_u5.js'), 'utf8');
...
const MAP = Array.from({ length: 14 }, (_, i) => ['u5_l' + String(i + 1).padStart(2, '0'), String(i + 1)]);
const NO_REVIEW = ['u5_l01'];
...
T('부팅 + u5 14차시 로드', () => {
  ...
  ok(keys.length === 14, 'u5 차시 ' + keys.length);
});
...
for (const [key, arg] of MAP) {
  T(key + ' 요소 렌더', () => {
    ...
    ok(/마음|짐작|띄어|낱말|읽/.test(ALL), '③ 마음/띄어읽기 서사 흔적 없음');
  });
}
...
T('저작권 가드 — 지도서 제재 미노출', () => {
  ...
  delete require.cache[require.resolve(path.join(TDIR, 'data/g2_korean_u5.js'))];
  require(path.join(TDIR, 'data/g2_korean_u5.js'));
  const L = global.window.LESSONS;
  const BANNED_SRC = ['밤 다섯 개', '자전거 타기 성공', '강아지 돌보기', '예린이', '세상에서 가장 힘이 센 말', '여우와 두루미'];
  const bad = [];
...
T('offline 짝 활동 전 차시 존재(국어 필수 1)', () => {
  ...
  require(path.join(TDIR, 'data/g2_korean_u5.js'));
  const L = global.window.LESSONS;
  const bad = Object.keys(L).filter(k => k.startsWith('u5_')).filter(k => !L[k].slides.some(s => s.block === 'offline_activity'));
...
T('review 존재(l01 제외 13차시)·exit 전 차시 존재', () => {
  ...
  require(path.join(TDIR, 'data/g2_korean_u5.js'));
  const L = global.window.LESSONS;
  const badRv = Object.keys(L).filter(k => k.startsWith('u5_') && !NO_REVIEW.includes(k)).filter(k => !L[k].slides.some(s => s.block === 'review' && s.data.items));
  const badEx = Object.keys(L).filter(k => k.startsWith('u5_')).filter(k => !L[k].slides.some(s => s.block === 'exit_ticket'));
