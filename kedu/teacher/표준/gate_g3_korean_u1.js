...
T('CURRICULUM ↔ LESSONS 정합 (u1 블록 8항목 등재 · ready 8 = u1 완주)', () => {
  /* ⚠️ 2026-08-21 u2 개통으로 CURRICULUM에 단원이 둘이 됐다. 전체를 긁으면 u2 항목이 섞여
     ...
     2026-08-24 u3(7항목) 개통으로 다시 3/22로 올렸다. 다음 단원(u4) 개통 때도 이 잘라내기는 그대로 두면 된다. */
...
T('⚠️ 허브 "3_korean" 등재 (u1 8 + u2 7 + u3 7 + u4 7 + u5 8 + u6 8 = units 6 · lessons 45)', () => {
  ...
  /* ⚠️ 2026-08-21 u2(7항목) 개통으로 1/8 -> 2/15로 올렸다. u1 게이트가 옛 수를 못 박고 있어
     ...
     2026-08-24 u3·u4·u5 개통으로 5/37, 2026-08-25 u6(8항목) 개통으로 6/45까지 올렸다. 다음 단원(u7) 개통 때도 이 두 수와 아래 재계산 줄을 함께 올릴 것 — 수학 라인에서 매 단원 겪은 대목이다. */
  ok(+m[1] === 6, 'units ' + m[1]);
  ok(+m[2] === 45, 'lessons ' + m[2]);
  ok(+m[2] === KEYS.length + 7 + 7 + 7 + 8 + 8, '허브 lessons ↔ u1 8 + u2 7 + u3 7 + u4 7 + u5 8 + u6 8 어긋남');
