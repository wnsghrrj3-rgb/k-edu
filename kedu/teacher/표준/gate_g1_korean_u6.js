/* gate_g1_korean_u6.js — L3 게이트 (g1 국어 u6 「또박또박 읽어요」 국어 §7 증보 13차시).
   ...
   실행: NODE_PATH=/home/claude/.jsdom/node_modules node gate_g1_korean_u6.js */
'use strict';
...
const DATA = fs.readFileSync(path.join(TDIR, 'data/g1_korean_u6.js'), 'utf8');
...
const MAP = Array.from({ length: 13 }, (_, i) => ['u6_l' + String(i + 1).padStart(2, '0'), String(i + 1)]);
...
T('부팅 + u6 13차시 로드', () => {
  ...
  ok(keys.length === 13, 'u6 차시 ' + keys.length);
});
...
for (const [key, arg] of MAP) {
  T(key + ' 요소 렌더', () => {
    ...
    ok(/문장|부호|마침표|쉼표|읽|또박|목소리|띄어/.test(ALL), '③ 단원 서사(읽기·부호) 흔적 없음');
  });
}
...
T('교과서 제재 표지 전 차시 미노출 (저작권 가드 자체검증)', () => {
  ...
  delete require.cache[require.resolve(path.join(TDIR, 'data/g1_korean_u6.js'))];
  require(path.join(TDIR, 'data/g1_korean_u6.js'));
