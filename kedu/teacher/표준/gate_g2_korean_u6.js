/* gate_g2_korean_u6.js — L4 게이트 (g2 국어 u6 「자신의 생각을 표현해요」 국어 §7 증보 15차시).
   ...
   실행: NODE_PATH=/home/claude/.jsdom/node_modules node gate_g2_korean_u6.js */
'use strict';
const fs = require('fs');
...
const DATA = fs.readFileSync(path.join(TDIR, 'data/g2_korean_u6.js'), 'utf8');
...
const MAP = Array.from({ length: 15 }, (_, i) => ['u6_l' + String(i + 1).padStart(2, '0'), String(i + 1)]);
...
T('부팅 + u6 15차시 로드', () => {
  ...
  ok(keys.length === 15, 'u6 차시 ' + keys.length);
});
...
for (const [key, arg] of MAP) {
  T(key + ' 요소 렌더', () => {
    ...
    ok(/생각|까닭|중요|제목|중심|읽/.test(ALL), '③ 생각/중요 내용 서사 흔적 없음');
  });
}
...
T('저작권 가드 — 지도서 제재 미노출', () => {
  ...
  delete require.cache[require.resolve(path.join(TDIR, 'data/g2_korean_u6.js'))];
  require(path.join(TDIR, 'data/g2_korean_u6.js'));
  const L = global.window.LESSONS;
  const BANNED_SRC = ['공공장소에서의 예절', '줄넘기의 좋은 점', '나무뿌리는 무슨 일을 할까', '나무 노래', '수연이네 가족회의', '누구를 보낼까요', '금덩이를 버린 형제', '토끼와 거북', '토끼의 재판', '저마다 다른 동물의 생김새'];
  const bad = [];
  Object.keys(L).filter(k => k.startsWith('u6_')).forEach(k => {
...
T('offline 짝 활동 전 차시 존재(국어 필수 1)', () => {
  ...
  require(path.join(TDIR, 'data/g2_korean_u6.js'));
  const L = global.window.LESSONS;
  const bad = Object.keys(L).filter(k => k.startsWith('u6_')).filter(k => !L[k].slides.some(s => s.block === 'offline_activity'));
...
T('review 존재(l01 제외 14차시)·exit 전 차시 존재', () => {
  ...
  require(path.join(TDIR, 'data/g2_korean_u6.js'));
  const L = global.window.LESSONS;
  const badRv = Object.keys(L).filter(k => k.startsWith('u6_') && !NO_REVIEW.includes(k)).filter(k => !L[k].slides.some(s => s.block === 'review' && s.data.items));
  const badEx = Object.keys(L).filter(k => k.startsWith('u6_')).filter(k => !L[k].slides.some(s => s.block === 'exit_ticket'));
