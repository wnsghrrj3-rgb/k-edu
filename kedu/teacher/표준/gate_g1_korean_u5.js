/* gate_g1_korean_u5.js — L3 게이트 (g1 국어 u5 「반갑게 인사해요」 국어 §7 증보 12차시).
   ...
   실행: NODE_PATH=/home/claude/.jsdom/node_modules node gate_g1_korean_u5.js */
'use strict';
...
const DATA = fs.readFileSync(path.join(TDIR, 'data/g1_korean_u5.js'), 'utf8');
...
const MAP = Array.from({ length: 12 }, (_, i) => ['u5_l' + String(i + 1).padStart(2, '0'), String(i + 1)]);
...
T('부팅 + u5 12차시 로드', () => {
  ...
  ok(keys.length === 12, 'u5 차시 ' + keys.length);
});

console.log('═══ B. 증보 12차시 삽입 요소 실렌더 (활동층·offline·exit·review) ═══');
for (const [key, arg] of MAP) {
  T(key + ' 요소 렌더', () => {
    ...
    ok(/인사|안녕|고마워|걸음|소리|말/.test(ALL), '③ 단원 서사(인사·연음) 흔적 없음');
  });
}

console.log('═══ C. 회귀 (전 12차시 openShow 무손상) ═══');
for (const [key, arg] of MAP) {
  T('회귀 ' + key, () => {
    const W2 = boot();
...
T('교과서 제재 표지 전 차시 미노출 (저작권 가드 자체검증)', () => {
  ...
  delete require.cache[require.resolve(path.join(TDIR, 'data/g1_korean_u5.js'))];
  require(path.join(TDIR, 'data/g1_korean_u5.js'));
