/* gate_g2_korean_u8.js — L4 게이트 (g2 국어 u8 「다양한 작품을 감상해요」 국어 §7 증보 15차시).
   ...
   실행: NODE_PATH=/home/claude/.jsdom/node_modules node gate_g2_korean_u8.js */
'use strict';
const fs = require('fs');
...
const DATA = fs.readFileSync(path.join(TDIR, 'data/g2_korean_u8.js'), 'utf8');
...
const MAP = Array.from({ length: 15 }, (_, i) => ['u8_l' + String(i + 1).padStart(2, '0'), String(i + 1)]);
const NO_REVIEW = ['u8_l01'];

function renderAll(w, lessonArg) {
  w.Teacher.openShow('8', String(lessonArg));
...
T('부팅 + u8 15차시 로드', () => {
  W = boot();
  const keys = Object.keys(W.LESSONS).filter(k => k.startsWith('u8_'));
  ok(keys.length === 15, 'u8 차시 ' + keys.length);
});

console.log('═══ B. 증보 15차시 삽입 요소 실렌더 (활동층·offline·exit·review) ═══');
for (const [key, arg] of MAP) {
  T(key + ' 요소 렌더', () => {
    ...
    ok(/작품|감상|시|낭송|이야기|인형극|느낌|마음/.test(ALL), '③ 작품/감상 서사 흔적 없음');
  });
}

console.log('═══ C. 회귀 (전 15차시 openShow 무손상) ═══');
for (const [key, arg] of MAP) {
  T('회귀 ' + key, () => {
    const W2 = boot();
    W2.Teacher.openShow('8', String(arg));
...
T('저작권 가드 — 지도서 제재 미노출', () => {
  ...
  delete require.cache[require.resolve(path.join(TDIR, 'data/g2_korean_u8.js'))];
  require(path.join(TDIR, 'data/g2_korean_u8.js'));
  const L = global.window.LESSONS;
  const BANNED_SRC = ['신호', '우산 사용법', '다툰 날', '개구리와 두꺼비', '누가 더 섭섭했을까', '재강이 구출 작전'];
  const bad = [];
  Object.keys(L).filter(k => k.startsWith('u8_')).forEach(k => {
...
T('offline 짝 활동 전 차시 존재(국어 필수 1)', () => {
  ...
  require(path.join(TDIR, 'data/g2_korean_u8.js'));
  const L = global.window.LESSONS;
  const bad = Object.keys(L).filter(k => k.startsWith('u8_')).filter(k => !L[k].slides.some(s => s.block === 'offline_activity'));
  ok(bad.length === 0, 'offline 누락: ' + bad.join(','));
});
T('review 존재(l01 제외 14차시)·exit 전 차시 존재', () => {
  global.window = { LESSONS: {} };
  delete require.cache[require.resolve(path.join(TDIR, 'data/g2_korean_u8.js'))];
  require(path.join(TDIR, 'data/g2_korean_u8.js'));
  const L = global.window.LESSONS;
  const badRv = Object.keys(L).filter(k => k.startsWith('u8_') && !NO_REVIEW.includes(k)).filter(k => !L[k].slides.some(s => s.block === 'review' && s.data.items));
  const badEx = Object.keys(L).filter(k => k.startsWith('u8_')).filter(k => !L[k].slides.some(s => s.block === 'exit_ticket'));
