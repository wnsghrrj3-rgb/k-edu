/* augment5_lib.js — 2세대 7요소 증보 공용기 v3(g2 수학용). 2026-09-22 베프. augment4_lib 후속.
...
function insertBefore(src, key, beforeId, slideObj) {
  ...
  const seg = src.slice(rg[0], rg[1]);
  const re = new RegExp('\\n( {4,8})\\{\\n\\s+"id": "' + beforeId + '"'); const m = seg.match(re); if (!m) return null;
  const I = m[1];
  const obj = '\n' + I + JSON.stringify(slideObj, null, 2).split('\n').join('\n' + I) + ',';
