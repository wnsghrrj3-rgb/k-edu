/* ============================================================================
   케이랩 도구 모듈 — 십 배열판 (tenframe) v3
   초점 (1학년 수 감각·가르기모으기) = 10을 한눈에, 가르고 모으기.
   v2: 자유탐구 / 미션 / 퀴즈 3모드 (KLab.ui 표준).
   v3: 학년 칸(low/mid/high) — D칸 표상 전환 사다리.
     · 저 = 한 판(10) 고정·★빈 칸 유령 점으로 "○과 ○을 모으면 10"(보수)을
            눈으로 보는 저학년 닻·일상어·퀴즈 숨김.
     · 중 = 두 판(20)·십몇=10과 몇·가르기 모으기 명시·퀴즈(개수+10의 짝).
     · 고 = 기존 전부 유지(config frames/max·count/toTen 퀴즈·자유 가르기).
...
(function () {
  ...
  window.KLab.register('tenframe', function (el, config) {
    ...
    var bands=ui.gradeBands({grade:grade,locked:!!config.grade,onChange:function(g){
      ...
      blue={}; mStep=0; mDone=false; mLock=false;
    ...
    function render(){
      ...
      for(var f=0;f<shown;f++){
        ...
        for(var r=0;r<2;r++)for(var c=0;c<5;c++){
          ...
          else if(ghost&&f===0&&gk<10){svg.appendChild(svgEl('circle',{cx:cxp+cell/2,cy:cyp+cell/2,r:cell*0.30,fill:'none',stroke:'#BFD4EA','stroke-width':3,'stroke-dasharray':'4 5','data-ghost':gk}));}
          gk++;
        }
      ...
      if(mode==='quiz'){ st.innerHTML='<span style="font-size:22px;color:#5a7894;">아래에서 답을 골라 누르세요</span>'; }
        ...
        if(G().ghost){
          ...
          st.innerHTML='<span style="font-size:46px;color:#1565C0;">'+shownNum+'</span>'
            ...
            +(shownNum<10?'<div style="font-size:26px;margin-top:6px;color:#12B886;">빈 자리 '+toTen+'개 — '+toTen+'개만 더 모으면 <b>10</b>!</div>':(shownNum===10?'<div style="font-size:26px;margin-top:6px;color:#0CA678;">한 판 꽉 찼어요 — 10!</div>':''));
        } else {
          st.innerHTML='<span style="font-size:46px;color:#1565C0;">'+shownNum+'</span>'
    ...
    function checkMission(orangeCount){
      ...
      if(_M[mStep].check(num,orangeCount)){
        ...
        setTimeout(function(){
          mLock=false;
          if(mStep<curMissions().length-1){mStep++;num=0;blue={};}
          else mDone=true;
          build();
        },1500);
      }
