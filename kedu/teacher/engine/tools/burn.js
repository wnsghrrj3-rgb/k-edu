...
(function () {
  ...
  window.KLab.register('burn', function (el, config) {
    ...
    var exp; // 'cond' | 'ext'
    var cd, ex;
    function cdReset(){ cd={ lit:false, cup:[false,false], oxy:[100,100], out:[false,false], chips:{fuel:false,oxy:false,temp:false} }; stopRaf(); }
      ...
      if(MISSIONS[mStep].check()){
        ...
        setTimeout(function(){
          ...
          if(mStep<MISSIONS.length-1){
            mStep++;
            var keep=(MISSIONS[mStep].exp===exp);
    ...
    function ctrlRow(){
      if(exp==='cond'){
        return '<div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center;margin-bottom:10px;">'
          +'<button class="bn-btn" data-act="light" style="'+btn+'background:#fff;color:'+C.hot+';border-color:'+C.hot+';">🔥 두 촛불 켜기</button>'
          +'<button class="bn-btn" data-act="cup0" style="'+btn+'background:#fff;color:'+C.blue+';border-color:'+C.blue+';">🥛 작은 컵 덮기</button>'
    ...
    function candle(x,y,lit,outSmoke){
      var h='<rect x="'+(x-22)+'" y="'+y+'" width="44" height="110" rx="9" fill="#FFF3BF" stroke="#E9C46A" stroke-width="3"/>'
        +'<line x1="'+x+'" y1="'+y+'" x2="'+x+'" y2="'+(y-12)+'" stroke="#5a4632" stroke-width="4"/>';
      if(lit)h+=flame(x,y-10,true);
      else if(outSmoke)h+='<path d="M '+x+' '+(y-16)+' q -8 -16 2 -30 q 9 -13 1 -26" stroke="#ADB5BD" stroke-width="5" fill="none" stroke-linecap="round" opacity="0.8"/>';
      return h;
    }
    function drawCond(svg){
      ...
      for(var i=0;i<2;i++){
        var x=POS[i];
        h+=candle(x,262,cd.lit&&!cd.out[i],cd.out[i]);
        if(cd.cup[i]){
          var c=CUP[i];
        ...
        h+='<text x="'+x+'" y="425" text-anchor="middle" font-size="18" font-weight="800" fill="'+C.ink+'" font-family="inherit">'+CUP[i].nm+(cd.out[i]?' — 꺼짐!':'')+'</text>';
      }
      // 조건 칩 3개
    ...
    function renderStatus(){
      ...
      if(pic==='cond'){
        if(cd.out[0]&&cd.out[1]) msg='<span style="color:'+C.good+';font-size:19px;">큰 컵이 더 오래 탔죠? 산소가 더 많았으니까요!</span>';
        ...
        else if(cd.lit) msg='<span style="color:'+C.ink+';font-size:19px;">조건 칩을 확인하고, 컵을 덮어 봐요!</span>';
