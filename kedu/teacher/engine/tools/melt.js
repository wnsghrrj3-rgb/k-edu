/* ============================================================================
   케이랩 도구 — 상태변화 (melt)  ★거시(실사) + [클릭] 미시(입자) 펼침 듀얼뷰
   기본: 실사 비커(녹고 끓고)만 보임.  "🔬 입자 보기" 누르면 옆에 분자 패널이
   스르륵 펼쳐져 같은 온도의 입자 배열·운동을 나란히 보여준다(다시 누르면 접힘).
   하나의 슬라이더(−20→120℃)로 거시·미시 완전 동기.
...
(function () {
  ...
  window.KLab.register('melt', function (el, config) {
    ...
    var VB_NARROW=540, VB_WIDE=1120;
    ...
    el.innerHTML=
      ...
      +'.mlt-micro{font-size:20px;padding:11px 20px;border-radius:15px;border:3px solid #7048E8;background:#fff;color:#7048E8;cursor:pointer;font-weight:800;font-family:inherit;line-height:1;}'
      +'.mlt-micro.on{background:#7048E8;color:#fff;}'
      +'</style>'
      ...
      +'<div style="display:flex;justify-content:center;margin-bottom:12px;">'
        +'<button class="mlt-micro" data-act="micro">🔬 입자 보기</button>'
      +'</div>'
    ...
    var microBtn=el.querySelector('[data-act="micro"]');

    var svg=S('svg',{viewBox:'0 0 '+VB_NARROW+' 480',width:'100%',style:'display:block;height:auto;'}); stageHost.appendChild(svg);
    ...
    function title(x,txt,sub,parent){ var g=S('g');
      ...
      (parent||svg).appendChild(g); return g;
    }
    ...
    // 입자 패널(미시) — 기본 숨김, 토글로 펼침
    var microGroup=S('g',{opacity:0,display:'none'}); svg.appendChild(microGroup);
    title(MX+MW/2,'🔬 입자의 모습','분자의 배열과 운동',microGroup);
    microGroup.appendChild(S('rect',{x:MX,y:MY,width:MW,height:MH,rx:20,fill:'#f4faff',stroke:'#bcd6ec','stroke-width':3}));
    var dividerArrow=S('text',{x:(BX+BW+MX)/2,y:BB-110,'font-size':30,fill:'#9bb4cc','text-anchor':'middle'}); dividerArrow.textContent='↔'; microGroup.appendChild(dividerArrow);
    var molLayer=S('g',{'clip-path':'url(#mMicroClip)'}); microGroup.appendChild(molLayer);
    var parts=[];
    (function buildParts(){ var startX=MX+MW/2-(COLS-1)*GX/2, startY=(MB-26)-(ROWS-1)*GY;
    ...
    var cur={e:6,t:derive(6).t,iceH:1,water:0,vbW:VB_NARROW,microOp:0}, bubbles=[],steams=[],drips=[],frame=0,auto=false,lastPhase='solid',tDrip=0,raf=null;
    var showMicro=false, vbTarget=VB_NARROW;
    ...
    function spawnBubble(){ var c=S('circle',{cx:BX+30+Math.random()*(BW-60),cy:BB-8,r:2+Math.random()*4,fill:'#fff',opacity:0}); bubbleLayer.appendChild(c); bubbles.push({el:c,y:BB-8,vy:0.8+Math.random()*1.4}); }
    function spawnSteam(x){ var e=S('ellipse',{cx:x,cy:BY-4,rx:16,ry:13,fill:'url(#mSteam)',filter:'url(#mBlur)',opacity:0}); steamLayer.appendChild(e); steams.push({el:e,x:x,y:BY-4,vy:0.7+Math.random()*0.7,drift:(Math.random()-0.5)*0.5,life:0,max:90+Math.random()*60}); }
    function spawnDrip(){ var x=ICE_X+ICE_W/2+(Math.random()-0.5)*ICE_W*0.7, topY=ICE_BASE-cur.iceH*ICE_FULLH+6; var im=S('image',{x:x-11,y:topY,width:22,height:22}); href(im,WATER); dripLayer.appendChild(im); drips.push({el:im,y:topY,vy:1.2+Math.random()*0.6}); }

    function microStep(mp){ var i,j,p,q,dx,dy,d,md=PR*2-2; var L=MX+22,R=MX+MW-22,T=MY+22,Bm=MB-22;
      for(i=0;i<NP;i++){ p=parts[i]; p.vx+=(Math.random()-0.5)*mp.thermal; p.vy+=(Math.random()-0.5)*mp.thermal;
        if(mp.latticeK>0){ p.vx+=(p.hx-p.x)*mp.latticeK; p.vy+=(p.hy-p.y)*mp.latticeK; } p.vy+=mp.gravity; p.vx*=mp.damp; p.vy*=mp.damp; p.x+=p.vx; p.y+=p.vy; }
      ...
      for(i=0;i<NP;i++){ p=parts[i]; if(p.x<L){p.x=L;p.vx*=-0.5;} if(p.x>R){p.x=R;p.vx*=-0.5;} if(p.y<T){p.y=T;p.vy*=-0.5;} if(p.y>Bm){p.y=Bm;p.vy*=-0.5;}
    ...
    function render(){
      ...
      if(iceImg.getAttribute('opacity')!=='0') iceImg.setAttribute('opacity',iceOp);
    ...
    function loop(){ frame++;
      ...
      var m=render();
      // 듀얼뷰 펼침/접힘
      cur.vbW+=(vbTarget-cur.vbW)*0.16; svg.setAttribute('viewBox','0 0 '+cur.vbW.toFixed(1)+' 480');
      cur.microOp+=((showMicro?1:0)-cur.microOp)*0.16;
      if(cur.microOp>0.01){ microGroup.setAttribute('display',''); microGroup.setAttribute('opacity',cur.microOp.toFixed(2)); }
      else { microGroup.setAttribute('opacity','0'); if(!showMicro) microGroup.setAttribute('display','none'); }
      if(showMicro) microStep(microParams(m));
      // 거시 연출
      if(m.phase==='boil'&&frame%3===0) spawnBubble();
      if(cur.t>92&&frame%9===0&&cur.water>0.05) spawnBubble();
      var topY=waterTopY(cur.water);
      for(var i=bubbles.length-1;i>=0;i--){ var b=bubbles[i]; b.y-=b.vy; b.vy+=0.02; b.el.setAttribute('cy',b.y); b.el.setAttribute('opacity',Math.min(0.7,+b.el.getAttribute('opacity')+0.06)); if(b.y<=topY+2){ if(b.el.parentNode)b.el.parentNode.removeChild(b.el); bubbles.splice(i,1); } }
      if(m.vapor>0.02&&frame%Math.max(3,Math.round(9-m.vapor*6))===0) spawnSteam(BX+BW/2+(Math.random()-0.5)*BW*0.55);
      for(var j=steams.length-1;j>=0;j--){ var s=steams[j]; s.life++; s.y-=s.vy; s.x+=s.drift; var k=s.life/s.max; s.el.setAttribute('cx',s.x.toFixed(1)); s.el.setAttribute('cy',s.y.toFixed(1)); s.el.setAttribute('opacity',(Math.sin(Math.min(1,k)*Math.PI)*0.7).toFixed(2)); var sc=1+k*1.3; s.el.setAttribute('rx',(16*sc).toFixed(1)); s.el.setAttribute('ry',(13*sc).toFixed(1)); if(s.life>=s.max){ if(s.el.parentNode)s.el.parentNode.removeChild(s.el); steams.splice(j,1); } }
      if(m.phase==='melt'&&m.iceH>0.03){ tDrip++; if(tDrip%20===0){ spawnDrip(); snd('tap'); } }
      var floorY=waterTopY(cur.water)-4;
      for(var d2=drips.length-1;d2>=0;d2--){ var dp=drips[d2]; dp.vy+=0.16; dp.y+=dp.vy; dp.el.setAttribute('y',dp.y.toFixed(1)); if(dp.y>=floorY){ if(dp.el.parentNode)dp.el.parentNode.removeChild(dp.el); drips.splice(d2,1); } }
    ...
    microBtn.addEventListener('click',function(){ showMicro=!showMicro; vbTarget=showMicro?VB_WIDE:VB_NARROW;
      microBtn.classList.toggle('on',showMicro); microBtn.textContent=showMicro?'🔬 입자 숨기기':'🔬 입자 보기'; snd(showMicro?'select':'tap'); });
