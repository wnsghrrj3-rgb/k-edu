...
window.MK_SEC = (() => {
  ...
  /* ---------- 사진형 더미 (실서비스풍 추상 그라데이션 — 회색 박스 금지) ---------- */
  const PH = {
    a: 'linear-gradient(135deg,#173A44 0%,#2E8C7F 55%,#8FCDB9 100%)',
    b: 'linear-gradient(150deg,#7C4A34 0%,#D9935F 58%,#F2D3AC 100%)',
    c: 'linear-gradient(140deg,#4C5C80 0%,#8CA0C4 55%,#DDE4F1 100%)',
    d: 'linear-gradient(160deg,#20313F 0%,#3E6A75 60%,#9CC3C0 100%)',
  };
  ...
  const cover = (P, { label, title, subtitle, meta }) => [
    box(0, 0, 100, 0.8, P.accent),
    box(8, 15.5, 3.6, 0.55, P.accent),
    t(13, 14, 60, 2.2, label, 700, P.accent),
    t(8, 25, 84, 12, title, 700),
    box(8, 57, 6.5, 0.8, P.accent2),
    t(8, 63.5, 64, 3.3, subtitle, 400),
    t(8, 87, 56, 2.3, meta, 500, P.mutedOnDark),
    box(88.6, 87.6, 1.6, 2.8, P.accent),
  ];

  /* 02 Agenda — 좌 대제목 · 우 번호 리스트 */
  const agenda = (P, { title, items }) => {
    const els = [t(8, 14, 30, 6.2, title, 700), box(8, 30, 5, 0.9, P.accent)];
    items.slice(0, 5).forEach((it, i) => {
      const y = 17 + i * 15;
      els.push(t(46, y, 6, 4, String(i + 1).padStart(2, '0'), 700, P.accent));
      els.push(t(55, y + 1, 37, 3.1, it, 600));
      els.push(box(46, y + 9.8, 46, 0.26, '#EDF0F4'));
    });
    return els;
  };
  ...
  const gallery = (P, { title, captions }) => {
    const els = [t(8, 11, 50, 5.8, title, 700), box(8, 24, 5, 0.9, P.accent)];
    const W = 27, G = 2.5, X0 = 8, phs = [PH.a, PH.c, PH.b];
    (captions || ['', '', '']).slice(0, 3).forEach((c, i) => {
      const x = X0 + i * (W + G);
      els.push(box(x, 31, W, 42, phs[i]));
      els.push(t(x, 77.5, W, 2.3, c, 500, P.mutedOnLight));
    });
    return els;
  };
  ...
  const chart = (P, { title, insight, bars }) => {
    const els = [t(8, 11, 60, 5.8, title, 700), t(8, 24, 52, 2.7, insight, 400, P.mutedOnLight)];
    const max = Math.max(...bars.map((b) => b.v));
    const X0 = 11, W = 13, G = 7.5, BASE = 82, HMAX = 42, SPAN = (W + G) * 4 - G + 2;
    els.push(box(X0 - 1, BASE - HMAX, SPAN, 0.24, '#F0F3F7'));
    els.push(box(X0 - 1, BASE - HMAX / 2, SPAN, 0.24, '#F0F3F7'));
    bars.slice(0, 4).forEach((b, i) => {
      ...
      els.push(t(x, BASE - h - 4.6, W, 2.8, String(b.v), 700, i === bars.length - 1 ? P.accent : undefined, 'center'));
      els.push(t(x, BASE + 3, W, 2.2, b.k, 500, P.mutedOnLight, 'center'));
    });
    els.push(box(X0 - 1, BASE, SPAN, 0.32, '#D9DFE8'));
    return els;
  };

  /* 07 Timeline — 가로 라인 + 노드 4개 */
  const timeline = (P, { title, steps }) => {
    const els = [t(8, 11, 60, 5.8, title, 700), box(8, 24, 5, 0.9, P.accent), box(8, 52, 84, 0.4, '#DDE3EB')];
    const N = Math.min(steps.length, 4), span = 84 / N;
    steps.slice(0, 4).forEach((s2, i) => {
      const x = 8 + i * span;
      els.push(box(x, 50.7, 1.7, 3, i === 0 ? P.accent2 : P.accent));
      els.push(t(x, 41.5, span - 4, 2.3, s2.k, 700, P.accent));
      els.push(t(x, 60.5, span - 4, 3, s2.title, 700));
      els.push(t(x, 68.5, span - 4, 2.3, s2.desc, 400, P.mutedOnLight));
    });
    return els;
  };
  ...
  const stats = (P, { title, items, note }) => {
    ...
    items.slice(0, 3).forEach((s2, i) => {
      const x = 8 + i * (W + G);
      els.push(box(x, 33, W, 36, P.soft));
      els.push(t(x, 40, W, 8, s2.v, 700, P.accent, 'center'));
      els.push(t(x, 58.5, W, 2.5, s2.k, 600, undefined, 'center'));
    });
  ...
  const pricing = (P, { title, plans }) => {
    const els = [t(8, 10, 50, 5.8, title, 700)];
    const W = 27, G = 2.5;
  ...
  const faq = (P, { title, items }) => {
    const els = [t(8, 11, 40, 5.8, title, 700), box(8, 24, 5, 0.9, P.accent)];
  ...
  const table = (P, { title, head, rows }) => {
    const els = [t(8, 10, 60, 5.8, title, 700), box(8, 24, 84, 7.5, P.dark)];
  ...
  const ending = (P, { title, lines }) => {
    ...
    (lines || []).slice(0, 3).forEach((l, i) => els.push(t(8, 61 + i * 6.5, 70, 2.6, l, 400, P.mutedOnDark)));
    return els;
  };
  ...
  const features = (P, { title, items }) => {
    const els = [t(8, 10, 60, 5.8, title, 700), box(8, 23, 5, 0.9, P.accent)];
    const W = 27, G = 2.5;
