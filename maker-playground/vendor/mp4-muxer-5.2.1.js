/* ============================================================
   mp4-muxer v5.2.1 — 자체 호스팅본 (R124)
   출처: https://www.npmjs.com/package/mp4-muxer  (npm mp4-muxer@5.2.1)
   원본 경로: package/build/mp4-muxer.js  — 무수정 그대로다.
   라이선스: MIT © 2023 Vanilagy — vendor/LICENSE-mp4-muxer.txt

   왜 자체 호스팅인가 (R124):
   학생이 마지막에 누르는 「영상으로 저장」 버튼이 남의 CDN 에 매달려 있었다.
   학교 방화벽은 우리가 못 고친다. 그래서 정본 muxerUrl 을 이 파일로 옮기고
   CDN 을 폴백으로 강등했다(그 반대가 아니다).

   ⚠ R124 발견 — 이 패키지에는 `mp4-muxer.min.js` 가 **없다.**
   종전 CDN 주소가 가리키던 .min.js 는 jsdelivr 가 즉석에서 만들어 주던
   것이지 패키지 안에 실존하는 파일이 아니었다. 즉 우리 내보내기는 파일이
   아니라 **CDN 의 기능** 하나에 매달려 있었다. 그래서 여기 두는 것은
   패키지에 실존하는 `mp4-muxer.js`(비압축)다 — 74KB, 전송은 gzip 이 먹는다.

   갱신 방법: `npm pack mp4-muxer@<ver>` → package/build/mp4-muxer.js 를
   이 머리말 아래에 붙이고, video.js EXPORT_SPEC 의 두 주소를 함께 올린다.
   ============================================================ */
"use strict";
var Mp4Muxer = (() => {
