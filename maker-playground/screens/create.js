...
window.MK_SCREENS.create = (() => {
  ...
  const step1 = () => {
    ...
    return `<div class="cf-head"><h2>무엇을 만들까요?</h2><p>만들 것을 고르면 나머지는 K-MAKER가 도와요</p></div>
      ...
      + (window.MK_SCENE3D ? window.MK_SCENE3D.SCENES.map((s) => `<button class="hv-type" data-scene3d="${s.id}"><span class="ico">${s.ico}</span><b>${s.title.replace(/ \(.*\)$/, '')}</b><small>3D 장면 영상 — 글자만 바꾸면 완성 (새 탭)</small></button>`).join('') : '')   /* R147 — 종류 카드 뒤 · 3D 상장 앞 */
