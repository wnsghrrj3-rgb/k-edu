/* ============================================================
   perf76.mjs (R76) — 순서를 바꾸는 조작 자체의 비용
   ------------------------------------------------------------
...
async function measure(rate) {
  ...
  const r = await pg.evaluate(async () => {
    ...
    /* ---- ▲ 한 번 재기 (R76 표적) ----
       순서만 한 칸 바뀌는 조작이다. 목록이 통째로 다시 만들어지는지,
       아니면 노드가 살아남는지가 갈림길이다. */
    const upOne = async (idx) => {
      const rt = root();
      const btn = rt.querySelector(`[data-vh-mup="${idx}"]`);
      if (!btn) return null;
      const mark = rt.querySelector('[data-vh-mrow="9"]');
      flush();
      const t0 = performance.now();
      btn.click();
      const block = performance.now() - t0;
      const paint = await new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(() => r(performance.now() - t0))));
      return { block, paint, kept: !!(mark && document.contains(mark)) };
    };
    const ups = [];
    for (const i of [5, 6, 7, 8]) {
      const u = await upOne(i);
      if (u) ups.push(u);
      await new Promise((r) => setTimeout(r, 150));
    }
    ...
    return {
      ...
      dragMs, wiringLost,
      upBlock: med(ups.map((u) => u.block)), upPaint: med(ups.map((u) => u.paint)), upKept: ups.map((u) => u.kept),
...
for (const rate of [1, 4, 6]) {
  ...
  console.log(`  ▲ 한 칸 올리기        멈춤 ${r.upBlock} ms / 그려지기까지 ${r.upPaint} ms · ${keep(r.upKept)}`);
