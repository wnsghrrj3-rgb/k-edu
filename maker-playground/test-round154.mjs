...
T('요소 28 = 텍스트 7 · 사진 자리 1 · 벡터 14 · 도형/단색 6 — PNG 통짜 0', () => { const k = (x) => sc.elements.filter((e) => e.kind === x).length; const ph = sc.elements.filter((e) => e.role === 'photo-slot').length; return sc.elements.length === 28 && k('text') === 7 && ph === 1 && k('vector') === 14 && !sc.elements.some((e) => e.src) || JSON.stringify({ n: sc.elements.length, t: k('text'), v: k('vector'), ph }); });
...
T('렌더(SVG 출력) 에 text 7 · 중첩 svg 14 · 경고 없음(글꼴 폴백 0)', () => { const dl = w.MK_RENDER.renderScene(sc, { noCache: true }); const svg = w.MK_RENDER.toSVG(dl); const nt = (svg.match(/<text /g) || []).length, nv = (svg.match(/<svg x=/g) || []).length; const miss = (dl.warnings || []).filter((x) => /missing-font|폰트 없음/.test(JSON.stringify(x))); return nt === 7 && nv === 14 && miss.length === 0 || JSON.stringify({ nt, nv, miss }); });
T('캔버스 DOM 에 요소 28 그려짐', () => w.document.querySelectorAll('[data-ws-el]').length === 28 || w.document.querySelectorAll('[data-ws-el]').length);
...
T('사진 넣으면(src) 풍경 15조각 visible:false, 다른 27요소 살아 있음', () => { const i = idxOf('photo-placeholder'); const el = sc.elements[i]; el.src = 'data:image/png;base64,iVBORw0KGgo='; const n = w.MK_TPLPKG.hideFallbacks(sc, el); const hidden = sc.elements.filter((e) => e.visible === false).length; return n === 15 && hidden === 15 && sc.elements.length === 28 || JSON.stringify({ n, hidden }); });
