...
| 28 | setup_kmovie_projects.sql | 2026-08-31 작성 · **미적용(준호 실행 대기)** | 케이무비 「내 작업」 — 편집 상태 JSON 을 교사 계정에 저장해 어느 기기에서든 목록에서 연다. `kmovie_projects(id uuid=클라이언트 생성, user_id, name, doc jsonb, dur_sec, clips, updated_at)` · RLS 본인 행만(authenticated) · doc 4MB 가드(원본 바이트 유입 방지). 원본 영상은 올리지 않는다(기기에 남음). 검산 `SELECT policyname FROM pg_policies WHERE tablename='kmovie_projects';` → p_kmovie_projects_owner. 미적용 상태에선 케이무비가 조용히 이 브라우저에만 저장(오류 없음) |
