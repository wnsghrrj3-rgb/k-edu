...
- 내 작업 (`engine/store.js`): 편집 상태를 이름 붙은 작업 파일로 여러 개 — 이 기기(IndexedDB, 400ms 자동) + 케이에듀 계정(Supabase `kmovie_projects`, **「저장」(Ctrl+S)을 눌렀을 때만**) + `.kmv` 파일 내보내기/가져오기. 시작할 때는 이 기기의 마지막 작업만 열고 계정 사본은 자동으로 열지 않는다(다른 기기 것은 「내 작업」에서 불러오기). 원본 영상은 작업 파일에 들어가지 않는다(프리미어처럼 기기에 남음) — 다른 기기에서 불러오면 원본을 이름+크기로 이 기기에서 찾아 붙이고, 없으면 그 클립은 빠지고 프로젝트 탭 「파일 다시 연결」로 그 영상을 골라 넣으면 돌아온다(그 상태에선 계정 저장 멈춤). 「내 작업」 모달 「📂 작업 폴더」: 폴더(File System Access, 크롬·엣지 데스크톱)를 한 번 고르면 「저장」 때마다 `<작업 이름>.kmv`(편집 상태만, 수십 KB — 영상 바이트 0)를 그 폴더에도 써 둔다(폰·파이어폭스는 내려받기). 옛 단일 자동 저장은 「이전 작업」으로 이관
- 프로 편집기 단골 기술 1차: **속도 램프**(클립 패널 「램프」 — 슬로·타임랩스로 부드럽게 들어가고 나옴, 클립 길이 불변·소리는 playbackRate 곡선으로 같은 매핑), **잡음 줄이기**(클립 패널 「잡음」 약/강 — 그 원본의 가장 조용한 1.5초를 잡음 지문으로 삼는 스펙트럴 게이트, FFT 1024/hop 256, 미리보기=내보내기), **색 맞춤**(룩 패널 — 그레이 월드 채널 게인으로 클립 간 색 온도 통일)
- **흔들림 잡기**(클립 패널 「흔들림」 약/강 — 손떨림 보정): 분석이 원본을 훑을 때 96×54 회색 그림의 세로줄·가로줄 합(적분 투영)을 ±8칸 맞춰 프레임 사이 이동량을 같이 재 둔다(**추가 디코드 0**). 이동량을 쌓은 궤적을 넓게 평균 내고(약 0.7초·강 1.8초) 그 차이만큼 화면을 반대로 민다 — 화면을 미리 조금 키워(약 1.099×·강 1.25×) 가장자리에 빈틈이 안 생기게 하고, 장면 경계(diff>0.10)에서는 궤적을 끊는다. 미리보기·내보내기 같은 경로(drawFit fill.zoom/sx/sy). 분석이 끝나기 전에는 적용하지 않음
