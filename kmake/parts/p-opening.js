/* 오프닝 타이틀 — 엠블럼이 돌아 들어온 뒤 학교 이름이 중앙에서 한 글자씩 떠오르고 금선이 열린다 (6초) */
(function (g) {
  'use strict';
  ...
  K.register({
    ...
    fields: [
      ...
      { k: 'emblem',  label: '엠블럼', def: 'on', opts: ['on', 'off'] },
    ],
    assets: ['emblem-in.png', 'emblem.png'],
    draw: function (ctx, W, H, t, p, T) {
      ...
      var em = p.emblem !== 'off' && K.drawEmblem;        // 엠블럼이 켜지면 글자 묶음을 아래로 조금 내림
      var dy = em ? 70 * s : 0;

      if (p.backing !== 'none') K.backing(ctx, W, H, life(t, 0, 0.8, OUT_A, OUT_B) * 0.9, p.backing);

      /* 엠블럼 — 맨 먼저 돌아 들어와(0~2.6s) 학교 이름 위에 멈춤 */
      if (em) K.drawEmblem(ctx, t - 0.05, cx, cy - 250 * s + dy + lift, 300 * s, 1 - gone);
      ...
      if (eyA > 0.002) {
        K.drawText(ctx, p.eyebrow, cx, cy - 118 * s + dy + lift + (1 - seg(t, 1.15, 1.9)) * -10 * s, {
      ...
      K.drawText(ctx, title, cx, cy + 22 * s + dy + lift, {
      ...
      if (tgA > 0.002) {
        K.drawText(ctx, p.tagline, cx, cy + 128 * s + dy + lift + (1 - tgU) * 16 * s, {
